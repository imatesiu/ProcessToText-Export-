...
public class BPMN2XPDLConversion {
	...
	public static final String ID_SUB_PROCESS_SUBWF = "SubProcess-SUBWF_";
	...
	private void fillSwimlanes() {
		List<XpdlActivity> activityList = xpdl.getWorkflowProcesses().getList()
				.get(0).getActivities().getList();

		Map<String, String> map3 = new HashMap<String, String>();
		for (BPMNNode bpmnNode : bpmn.getNodes()) {
			if (bpmnNode.getParentSwimlane() != null) {
				map3.put("" + bpmnNode.hashCode(), ""
						+ bpmnNode.getParentSwimlane().hashCode());
			}
		}
		if (!map3.isEmpty()) {
			Swimlane bpmnPool = retrievePool(bpmn);
			XpdlPools xpdlPools = xpdl.getPools();
			XpdlPool xpdlPool = new XpdlPool(bpmnPool.getLabel());
			xpdlPool.setId(bpmnPool.getId().toString());
			xpdlPool.setBoundaryVisible("false");
			xpdlPool.setProcess(xpdl.getWorkflowProcesses().getList().get(0)
					.getId());
			...
			for (XpdlActivity xpdlActivity : activityList) {
				if (xpdlActivity.getNodeGraphicsInfos() != null) {
					List<XpdlNodeGraphicsInfo> infos = xpdlActivity
							.getNodeGraphicsInfos().getList();
					for (XpdlNodeGraphicsInfo xpdlNodeGraphicsInfo : infos) {
						xpdlNodeGraphicsInfo.setLaneId(map3.get(xpdlActivity
								.getId()));
					}
				}
			}
 ...
 */
	...
	private void fillXpdlActivities() {
		...
		for (SubProcess subProcess : bpmn.getSubProcesses()) {
			...
			activityBlockActivity.setActivitySetId(ID_SUB_PROCESS_SUBWF + subProcess.getId());
			...
			for (XpdlActivitySet tempXpdlActivitySet : mainWorkflowProcess.getActivitySets().getList()) {
				if (tempXpdlActivitySet.getId().equals(ID_SUB_PROCESS_SUBWF + subProcess.getId())) {
		...
		for (Flow flow : bpmn.getFlows()) {
			...
			if (flow.getParentSubProcess() == null) {
				...
				for (XpdlActivitySet tempActivitySet : mainWorkflowProcess.getActivitySets().getList()) {
					if (tempActivitySet.getId().equals(ID_SUB_PROCESS_SUBWF + flow.getParentSubProcess().getId())) {
	...
	private Xpdl fillXPDL() {
		...
		w1.setId(ID_MAIN_PROCESS_WF + bpmn.getLabel());
		...
		for (SubProcess subProcess : bpmn.getSubProcesses()) {
			...
			xpdlActivitySet.setId(ID_SUB_PROCESS_SUBWF + subProcess.getId());
		...
		// FILL XPDL SETTINGS
		xpdl.setId(ID_XPDL + bpmn.getLabel());
